...
const CurrentSongInformation = ({ song }) => {
  ...
  return `
    <audio src=${song.musicPath} id="audio-${song.id}" data-id="${
    song.id
  }" onEnded="appState.autopilotMode(this, '${stringify(
    song
  )}')" onloadeddata="${updateDuration({
    selectors: `#audio-${song.id},#seek-${song.id}, #seek-right-${song.id}, #duration`,
