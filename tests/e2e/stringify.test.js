...
describe('Convert data type to render prop string', function() {
    ...
    it('should correctly serialize a bigInt', () => {
        const input = 27n;
        const result = stringify(input);
        expect(result).toBe('_9s35Ufa7M67wghwT_&quot;__BigInt__27&quot;_9s35Ufa7M67wghwT_');
    });

    it('should correctly serialize Date', () => {
        const input = new Date()
        const result = stringify(input);
        expect(result).toBe(`_9s35Ufa7M67wghwT_&quot;${input.toISOString()}&quot;_9s35Ufa7M67wghwT_`);
    });

    it('should correctly serialize regular expressions', () => {
        const input = /\s*\$trigger\s*\(\s*([^,]+)\s*(?:,\s*([^,]+)\s*)?(?:,\s*{\s*([^{}$]+)\s*}\s*)?\s*\)/g;
        const result = stringify(input);
        expect(result).toBe('_9s35Ufa7M67wghwT_&quot;__RegExp__&#x2F;\\\\s*\\\\$trigger\\\\s*\\\\(\\\\s*([^,]+)\\\\s*(?:,\\\\s*([^,]+)\\\\s*)?(?:,\\\\s*{\\\\s*([^{}$]+)\\\\s*}\\\\s*)?\\\\s*\\\\)&#x2F;g&quot;_9s35Ufa7M67wghwT_');
    });

    it('should correctly serialize a undefined', () => {
        const input = undefined;
        const result = stringify(input);
        expect(result).toBe('_9s35Ufa7M67wghwT_&quot;__undefined__&quot;_9s35Ufa7M67wghwT_');
    });
    ...
    it('should correctly serialize a Map object', () => {
        ...
        input.set(1);
        const result = stringify(input);
        expect(result).toBe('_9s35Ufa7M67wghwT_{&quot;dataType&quot;:&quot;Map&quot;,&quot;value&quot;:[[1,&quot;__undefined__&quot;]]}_9s35Ufa7M67wghwT_');
    });
    ...
    it('should correctly serialize a function', () => {
        ...
        const input = () => new Set([1,3]);
        const result = stringify(input);
        const match = /__function__([^]*?)&quot;_/.exec(result);
        const randomId = match[1];
        const output = globalThis[password][`test_${randomId}`];
        expect(output.name).toBe('input');
    });
    ...
    it('should correctly serialize a string', () => {
        ...
        const result = stringify(input);
        expect(result).toBe('_9s35Ufa7M67wghwT_&quot;$render is the best JavaScript framework in the world&quot;_9s35Ufa7M67wghwT_');
    });

    it('should correctly serialize Symbol', () => {
        const input = Symbol.for('greatest');
        const result = stringify(input);
        expect(result).toBe('_9s35Ufa7M67wghwT_&quot;__symbol__Symbol(greatest)&quot;_9s35Ufa7M67wghwT_');
    });
